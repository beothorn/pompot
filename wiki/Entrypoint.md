...
- **pom.xml bootstrapping**: Pompot scans the working directory by default or the folder provided via `--parent=<path>`, parsing every descendant `pom.xml` into an in-memory JSON snapshot.
