...
- **pom.xml bootstrapping**: run Pompot inside the target workspace or pass `--parent=<path>` to parse every descendant `pom.xml` into a JSON snapshot (paths starting with `~/` expand to the user home).
